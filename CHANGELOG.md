...
Tests: `test-manifest-preflight.sh` (8 checks), `test-collateral-guard.sh` (7 checks) — both built from the run's own numbers (a 60-line edit inside a 2,060-line diff, 78 of 84 assets, the three real manifest paths). Full suite 45/45 hook-test files green; fixtures, ci-lint, and rule-index check clean.
