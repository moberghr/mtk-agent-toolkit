...
> The `/moberg:implement` command reads this file.
...
>
> **Decision rule:** If you're unsure, start with `fix`. It has a scope guard —
> if the change grows beyond 3 files, it will tell you to switch to `implement`.
...
Commands available:
  /moberg:implement  — Full feature loop (plan → implement → verify → review → fix → cleanup → learn)
  /moberg:update     — Pull latest toolkit from central repo
