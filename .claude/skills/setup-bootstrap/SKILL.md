...
For framework-specific guidance, see the `tech-stack-{stack}` skill (provided by the MTK plugin and resolved at runtime — not a file committed to this repo).
...
- Stack-specific references listed in the `tech-stack-{stack}` skill's `## Reference Files` section (MTK plugin, resolved at runtime)
````
