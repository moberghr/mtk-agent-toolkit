...
next Edit fails with "File has been modified since read". This matters more for
Python than for most stacks: `ruff check --fix` can rewrite imports and rename
bindings, so a mid-turn run does not just reflow whitespace under the model — it
can change what the file means between the read and the edit. Matchers are
regexes on **tool name** only — `Write(*.py)` is not valid syntax and silently
never fires; file-extension dispatch happens inside the hook.
