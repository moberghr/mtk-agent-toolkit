...
# constitution_file() — resolve which file carries the project's rules and the
# documented dependency names: AGENTS.md when it exists, is NOT generator-marked
# (the string `Auto-generated by MTK` within its first 10 lines — the same
# AGENTS_MARKER row 5 below checks), and CLAUDE.md is either absent or a shim
# (contains a line matching ^@AGENTS\.md); otherwise CLAUDE.md. A generator-marked
# AGENTS.md is the legacy references summary, never the constitution. The marker is
# matched on a captured variable, not piped into `grep -q` (S3.17). Duplicated per
# the standalone-script convention; same rule as constitution-digest.sh's copy.
constitution_file() {
  ...
  if [ -f AGENTS.md ] && [ "$marked" -eq 0 ] \
     && { [ ! -f CLAUDE.md ] || grep -q '^@AGENTS\.md' CLAUDE.md 2>/dev/null; }; then
    printf '%s\n' "AGENTS.md"
  else
    printf '%s\n' "CLAUDE.md"
  fi
