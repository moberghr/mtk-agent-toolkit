...
#      cross-reference identifier-shaped backtick tokens in CLAUDE.md + the
#      resolved constitution file (AGENTS.md in an inverted repo, via the same
#      constitution_file() rule; deduped to CLAUDE.md in a legacy one) +
...
#   5. AGENTS.md — first checked for the canonical constitution's
#      `<!-- mtk-setup` footer (the same stamp CLAUDE.md's version footer
#      uses): when present, classified by FOOTER VERSION DRIFT against
#      .claude/mtk-version.json, like row 3(a) — never diffed against
#      generate-agents-md.sh, which cannot reproduce hand/LLM-authored
#      content. Otherwise, checked for generate-agents-md.sh's own
#      auto-generated marker directly (grep, no subprocess): a hand-curated
#      AGENTS.md without either marker -> unknown, and the regenerator is
#      never invoked (generate-agents-md.sh would itself refuse to overwrite
#      it, so there is nothing meaningful to diff). When the legacy marker is
#      present, a copy of the real file seeds the temp target (so the
#      generator's `## Custom:` section harvesting sees the real content, not
#      an empty file) before regenerating into it via generate-agents-md.sh
#      --force and diffing against the real one; differs -> stale; script or
#      AGENTS.md absent -> unknown.
#   6. path-references — runs verify-references.sh over CLAUDE.md, AGENTS.md,
#      and .claude/rules/*.md; exit 3 (STALE lines) -> stale with the count;
#      script absent -> row is OMITTED (not `unknown`; this is the one
#      explicit exception to the general degradation rule).
...
constitution_file() {
  ...
  local _rc _a="AGENTS.md" _c="CLAUDE.md" _h="" _m=0
...
check_claude_md() {
  # Row label stays CLAUDE.md: this row covers the constitution PAIR (row 5 owns
  # AGENTS.md's own regeneration/footer verdict), and labelling it by the
  # resolved file would emit two identically-named rows in an inverted repo.
  # The reasons below name the file actually read.
  local doc="CLAUDE.md"
  if [ ! -f "$doc" ]; then
    add_row "$doc" "missing" "file not found"
    return
  ...
  local parsed status reason const_file
  # The dependency rescan must read the file that actually documents the stack.
  # In an inverted repo the Tech Stack / Project Profile sections live in
  # AGENTS.md and CLAUDE.md is a bare shim, so scanning CLAUDE.md alone finds no
  # dependency names at all and row 3 reports a vacuous "fresh".
  const_file="$(constitution_file)"
  parsed="$(python3 - "$const_file" <<'PY'
import sys; sys.stdout.reconfigure(newline="\n")  # LF even on Windows python3: bash parses this output
import glob
import json
import os
...
# The bootstrap footer is stamped on the RESOLVED constitution (AGENTS.md in an
# inverted repo), so reading CLAUDE.md here reported "no footer stamp — check
# skipped" on a repo whose constitution is genuinely version-stale, and counted
# that row as `fresh`. Read the same file part (b) scans.
const_doc = sys.argv[1] if len(sys.argv) > 1 else "CLAUDE.md"
const_text = read_text(const_doc)
reasons = []

# --- (a) version drift: constitution footer vs .claude/mtk-version.json -----
version_status = "skipped"
footer_match = re.search(r"mtk-setup:\s*v([0-9][0-9.]*)", const_text)
...
if os.path.isfile(version_file):
    ...
    if footer_match and installed:
        ...
        if stamped != installed:
            ...
            reasons.append(f"{const_doc} footer stamped v{stamped}, installed v{installed}")
        else:
            version_status = "fresh"
    else:
        reasons.append(f"{const_doc} has no mtk-setup footer stamp (version check skipped)")
else:
...
if not manifest_found:
    ...
    # CLAUDE.md, the resolved constitution file (AGENTS.md in an inverted repo,
    # already CLAUDE.md in a legacy one — deduped, so a legacy repo scans exactly
    # the same set as before), and every rule file.
    docs = ["CLAUDE.md"]
    if const_doc and const_doc not in docs:
        docs.append(const_doc)
    docs += sorted(glob.glob(".claude/rules/*.md"))
...
# --- Row 5: AGENTS.md --------------------------------------------------------
# Two distinct provenances now exist:
#   - the canonical, hand/LLM-authored constitution setup-bootstrap STEP 3
#     writes from root-agents-md-template.md, stamped with the same
#     `<!-- mtk-setup` footer CLAUDE.md's version stamp uses (row 3, part a):
#     classified by FOOTER VERSION DRIFT, never diffed against
#     generate-agents-md.sh's regeneration (which cannot reproduce curated
#     content and must never be invoked against it — the legacy diff branch
#     below is unreachable for this file because it lacks the legacy marker).
#   - the legacy generator marker ("Auto-generated by MTK") path: unchanged
#     regenerate-and-diff comparison.
#   - neither marker: hand-curated, "unknown" (unchanged).
...
AGENTS_TEMPLATE_FOOTER="<!-- mtk-setup"
if [ ! -f "$AGENTS_FILE" ]; then
  add_row "$AGENTS_FILE" "unknown" "AGENTS.md not found"
elif grep -qF "$AGENTS_TEMPLATE_FOOTER" "$AGENTS_FILE" 2>/dev/null; then
  if [ "$HAVE_PYTHON3" -ne 1 ]; then
    add_row "$AGENTS_FILE" "unknown" "python3 not available for footer version check"
  else
    parsed="$(python3 - "$AGENTS_FILE" <<'PY'
import sys; sys.stdout.reconfigure(newline="\n")  # LF even on Windows python3: bash parses this output
import json, os, re
...
footer_match = re.search(r"mtk-setup[:\s]*v([0-9][0-9.]*)", text)
version_file = ".claude/mtk-version.json"
if not os.path.isfile(version_file):
    print("unknown")
    print("no .claude/mtk-version.json (footer version check skipped)")
elif not footer_match:
    print("unknown")
    print("AGENTS.md has a mtk-setup footer with no parseable version")
else:
    installed = ""
    try:
        installed = json.load(open(version_file, encoding="utf-8")).get("version", "")
    ...
    if not installed:
        print("unknown")
        print("could not read installed version from .claude/mtk-version.json")
    elif stamped != installed:
        print("stale")
        print(f"AGENTS.md footer stamped v{stamped}, installed v{installed}")
    else:
        print("fresh")
        print("footer version matches installed")
PY
)"
    status="${parsed%%$'\n'*}"
    reason="${parsed#*$'\n'}"
    add_row "$AGENTS_FILE" "$status" "$reason"
  fi
elif [ ! -f "$GEN_AGENTS" ]; then
...
if [ -f "$VERIFY_REFS" ]; then
  vr_rc=0
  vr_out="$(bash "$VERIFY_REFS" CLAUDE.md AGENTS.md .claude/rules/*.md 2>/dev/null)" || vr_rc=$?
  case "$vr_rc" in
    0)
