...
# means GEN_MARKER's `Auto-generated by MTK` string appears within the file's
# first 10 lines — i.e. THIS script wrote it. That file is a references summary,
# not a constitution, so it must not be treated as canonical: without this clause
# the script would refuse to regenerate its own previous output in any repo whose
# CLAUDE.md is a shim. Legacy repos (constitution in CLAUDE.md, no AGENTS.md or a
# generator-marked one) resolve exactly as today. The marker is matched on a
# captured variable, not piped into `grep -q`, so this is not subject to the
# S3.17 SIGPIPE-under-pipefail class.
# Duplicated verbatim in generate-tool-configs.sh (standalone scripts); same rule
# as constitution-digest.sh's copy.
constitution_file() {
