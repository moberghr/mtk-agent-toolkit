...
# constitution_file() — resolve which file carries the numbered Critical Rules:
# AGENTS.md when it exists, is NOT generator-marked (the string
# `Auto-generated by MTK` within its first 10 lines — the marker
# generate-agents-md.sh / generate-tool-configs.sh stamp into what they write),
# and CLAUDE.md is either absent or a shim (contains a line matching
# ^@AGENTS\.md); otherwise CLAUDE.md. A generator-marked AGENTS.md is the legacy
# references summary, never the constitution. A legacy repo (no AGENTS.md, a
# marked one, or an unshimmed CLAUDE.md) resolves exactly as today. The marker is
# matched on a captured variable rather than through a pipe into `grep -q`
# (S3.17). Duplicated per-script by design (S3.3 standalone-script convention —
# these ship individually to target repos); keep it identical to
...
for f in .claude/rules/*.md "$CONST_FILE"; do
  [ -f "$f" ] || continue
...
for h in hooks/*.sh; do
  ...
  if ! grep -rqiF "$stem" .claude/rules/ "$CONST_FILE" 2>/dev/null \
     && ! grep -rqiF "$spaced" .claude/rules/ "$CONST_FILE" 2>/dev/null; then
