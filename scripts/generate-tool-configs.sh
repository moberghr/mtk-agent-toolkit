...
constitution_file() {
  ...
  local _a="AGENTS.md" _c="CLAUDE.md" _h="" _m=0
  if [ -f "$_a" ]; then
...
generate_pointer_file() {
  ...
  body="$(
    printf '# %s\n\n' "$title"
    printf '> Auto-generated by MTK. Regenerate with: `bash scripts/generate-tool-configs.sh --format %s`\n\n' "$format"
    # constitution_file() falls back to CLAUDE.md when NEITHER file exists, so a
    # pointer emitted in a repo with no constitution would name a file that is
    # not there — exactly the stranding this mode exists to avoid. Say so instead.
    if [ -f "$const_file" ]; then
      # shellcheck disable=SC2016 # backticks are literal markdown
      printf 'Canonical instructions: read `%s`.\n' "$const_file"
    else
      printf 'No constitution file found in this repo — run `/mtk-setup` to generate one.\n'
    fi
    if [ -n "$critical_rules" ]; then
      printf '\n## Critical Rules\n\n'
      printf '%s\n' "$critical_rules"
