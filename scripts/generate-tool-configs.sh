...
# Pointer mode (copilot/windsurf/cline): title, marker, a line naming
# AGENTS.md as the canonical instructions, and the `## Critical Rules` body
...
generate_pointer_file() {
  ...
  body="$(
    ...
    printf 'Canonical instructions: read `AGENTS.md`.\n'
    if [ -n "$critical_rules" ]; then
      printf '\n## Critical Rules\n\n'
      printf '%s\n' "$critical_rules"
    fi
  ...
  if [ "$total" -gt 40 ]; then
    ...
    {
      printf '%s\n' "$collapsed" | awk 'NR<=39'
      printf '<!-- truncated at the 40-line cap — full rules in AGENTS.md -->\n'
    } > "$output"
    echo "generate-tool-configs.sh: $output truncated at the 40-line cap — $dropped line(s) dropped; full rules in AGENTS.md" >&2
  else
    printf '%s\n' "$collapsed" > "$output"
  fi
  lines=$(wc -l < "$output" | tr -d ' ')
