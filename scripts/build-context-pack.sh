...
#   3. the resolved constitution file's `## Critical Rules` verbatim (AGENTS.md when it
#      exists, is not generator-marked, and CLAUDE.md is absent or a shim importing it
#      via `@AGENTS.md`; otherwise CLAUDE.md — same rule as constitution-digest.sh's
#      constitution_file(), duplicated here in python); every other `##` heading as a one-line TOC
...
# Constitution: resolved in bash by the shared resolver and passed in, exactly as
# the stack is above — one rule, one implementation (resolve-constitution.sh).
CONST_NAME=""
if [ -x "$MTK_ROOT/scripts/resolve-constitution.sh" ]; then
  CONST_NAME="$(cd "$ROOT_DIR" && bash "$MTK_ROOT/scripts/resolve-constitution.sh" 2>/dev/null || true)"
fi

python3 - "$ROOT_DIR" "$MTK_ROOT" "$STACK" "$SIDECAR" "$UUID" "$OUT" "$DRY" "$WARN_BYTES" "$CHANGE_MAP" "$CONST_NAME" <<'PY'
...
# 3. the resolved constitution file — resolved by scripts/resolve-constitution.sh
# (passed in as argv[10]); the fallback below keeps this script working if the
# resolver could not be located.
def constitution_file(root):
    passed = sys.argv[10] if len(sys.argv) > 10 else ""
    if passed:
        return os.path.join(root, passed), passed
    agents_path = os.path.join(root, "AGENTS.md")
    claude_path = os.path.join(root, "CLAUDE.md")
    if os.path.isfile(agents_path):
        marked = "Auto-generated by MTK" in "\n".join(read(agents_path).splitlines()[:10])
        if not marked and (
            not os.path.isfile(claude_path)
            or os.path.samefile(claude_path, agents_path)
            or re.search(r'^@AGENTS\.md', read(claude_path), re.M)
        ):
            return agents_path, "AGENTS.md"
    return claude_path, "CLAUDE.md"

const_path, const_name = constitution_file(root)
const_text = read(const_path)
if const_text:
    h, b = section(const_text, "Critical Rules")
    toc = [hh for l, hh, _ in sections(const_text) if l == 2]
    parts.append(f"## {const_name} — Critical Rules\n\n" + (b if b else "_(no `## Critical Rules` section)_") + f"\n\nOther {const_name} sections (read on demand): " + " · ".join(f"`{t}`" for t in toc) + "\n")
    inventory.append(f"{const_name} critical rules ({len(b)} chars) + TOC of {len(toc)} headings")
else:
    inventory.append(f"{const_name}: MISSING")
