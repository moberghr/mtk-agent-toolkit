...
# 3. the resolved constitution file (AGENTS.md-first; see constitution_file())
def constitution_file(root):
    """AGENTS.md when it exists, is NOT generator-marked, and CLAUDE.md is absent or
    a shim (a line matching ^@AGENTS\\.md); otherwise CLAUDE.md. Generator-marked
    means the string `Auto-generated by MTK` appears within the file's first 10
    lines — the marker generate-agents-md.sh / generate-tool-configs.sh stamp into
    every file they write. Such an AGENTS.md is the legacy references SUMMARY, never
    the constitution, so it must never be promoted over a real CLAUDE.md. Same rule
    as constitution-digest.sh's bash constitution_file(), duplicated per the
    standalone-script convention — this script ships individually to target repos,
    same as the others."""
