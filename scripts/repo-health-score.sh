...
# 1. Instructions file present and non-empty. AGENTS.md is the canonical
# constitution when it exists, is NOT generator-marked (the string
...
# ^@AGENTS\.md); otherwise CLAUDE.md, matching today's behavior for a legacy
# repo with only a CLAUDE.md constitution. A generator-marked AGENTS.md is the
# legacy references summary, never the constitution. Same rule as
# constitution-digest.sh's constitution_file(); the marker is matched on a
# captured variable, not through a pipe into `grep -q` (S3.17).
AGENTS_MARKED=0
if [[ -f AGENTS.md ]]; then
  AGENTS_HEAD10="$(awk 'NR<=10' AGENTS.md 2>/dev/null || true)"
  case "$AGENTS_HEAD10" in *"Auto-generated by MTK"*) AGENTS_MARKED=1 ;; esac
fi
INSTR_FILE=""
if [[ -f AGENTS.md ]] && [[ "$AGENTS_MARKED" -eq 0 ]] \
