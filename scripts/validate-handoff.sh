...
if [ "$FIELDS_ONLY" -eq 0 ]; then
  ...
  # A file produced by a tool rather than typed by a person is not undeclared
  # drift — it is expected collateral of a regenerator that ran. Requiring every
  # such path in the change_manifest trains the planner to enumerate machine
  # output, and a forgotten entry then reports as CRITICAL drift for a file no
  # human wrote (a 2026-09 field run lost a drift slot to `.claude/rules/INDEX.md`,
  # which is regenerated by a script and says so in its own header).
  is_derived() {
    case "$1" in
      ...
      *.Designer.cs|*.g.cs|*.g.i.cs|*.generated.*|*_pb2.py|*_pb2.pyi|*.pb.go) return 0 ;;
      *.min.js|*.min.css) return 0 ;;
      checksums.sha256|*/checksums.sha256) return 0 ;;
    esac
    # Otherwise trust the file's own banner: a generator that writes "generated by
    # / do not edit" in its first lines is declaring itself, in every language.
    [ -f "$1" ] || return 1
    head -n 5 "$1" 2>/dev/null \
      | grep -qiE '(auto-?generated|generated by|@generated|do not edit)' || return 1
    return 0
  }
  ...
  while IFS= read -r a; do
    [ -n "$a" ] || continue
    covered "$a" && continue
    if is_derived "$a"; then
      derived_files="${derived_files}${a}"$'\n'
    else
      extra_files="${extra_files}${a}"$'\n'
    fi
  done <<< "$actual_files"
  missing_files=""
  while IFS= read -r d; do
...
if [ -n "$derived_files" ]; then
  printf '## NOTE: derived artifacts touched but not declared (not drift)\n\n'
  printf '%s\n' "$derived_files" | sed 's/^/- `/; s/$/`/'
  printf '\nThese are machine-written (lockfile, snapshot, generated source, or a file\n'
  printf 'whose own header says it is generated). Confirm the regenerator that produced\n'
  printf 'them belongs to this change; do not add them to the change_manifest.\n\n'
fi
