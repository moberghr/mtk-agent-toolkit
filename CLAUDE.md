...
| Declare that subagent dispatch is unavailable | `MTK_SUBAGENT_DISPATCH=0` in `.claude/settings.local.json` env | Tells `implement`'s Phase 2.9 pre-flight up front that this session cannot dispatch implementer subagents — the harness does not expose `Agent`/`Workflow`, or a standing instruction forbids subagents the engineer did not ask for. HIGH/MAX then runs the **inline-MAX profile** (compensations C1–C3) instead of prescribing a path that cannot run and logging the same ceremony reduction once per run. It does **not** lower the rigor level: the level reflects the change's blast radius, the path reflects the session's tooling. Default (unset) probes per run |
| Force or skip the pre-flight baseline | `MTK_BASELINE_CAPTURE=1` / `=0` in `.claude/settings.local.json` env | `implement` Phase 2.9 captures build/test/typecheck state at the base commit before the first edit, so a failure found at batch 3 is attributable instead of needing a throwaway worktree to prove it pre-existing. On by default at rigor HIGH/MAX; `=1` forces it at any level, `=0` opts out — and then every later checkpoint must state that no baseline exists rather than implying one |
| Tune or disable the host-load probe | `MTK_HOST_LOAD_MAX=N` / `MTK_HOST_LOAD_PROBE=0` in `.claude/settings.local.json` env | `scripts/host-load-probe.sh` reports the 1-minute load per core and says `overloaded` above `MTK_HOST_LOAD_MAX` (default `2.0`). `implement` Phase 2.9 runs it before the first dispatch and `subagent-implementation` runs it before any killed-mid-batch respawn: an overloaded host routes the run to inline-MAX instead of dispatching implementers that the harness's 10-minute no-output watchdog will kill while they wait on a slow build (a 2026-09 field run lost two implementers and their respawn this way). `=0` skips the probe where the load figure means nothing (shared CI runners) |
| Cap parallel implementer waves | `MTK_BATCH_WAVE_MAX=N` in `.claude/settings.local.json` env | Phase 3 on the subagent path runs batches at the same `depends` level concurrently (the planning skill requires an edge between any two batches that share a file); this caps how many implementers one wave dispatches (default `3`) so a run cannot fan out past the org's concurrency or spend guard. `1` restores fully sequential batches |
| Tune the mid-run churn review thresholds | `MTK_CHURN_REVIEW_LINES` / `MTK_CHURN_HALT_LINES` in `.claude/settings.local.json` env | Net non-generated lines changed since the last review before `incremental-implementation` / `subagent-implementation` trigger an early review (default 300) or halt for `compliance-reviewer` (default 500). Defaults double at rigor HIGH/MAX, where every batch is already isolated and drift-checked and Phase 4 review is guaranteed. Generated files (lockfiles, `*.Designer.cs`, `*.g.cs`, migration snapshots, minified/bundled output) never count |
| Tune the collateral-churn thresholds | `MTK_COLLATERAL_WS_MIN` / `MTK_COLLATERAL_WS_RATIO` / `MTK_COLLATERAL_GEN_MIN` / `MTK_COLLATERAL_ASSET_PCT` / `MTK_COLLATERAL_ASSET_MIN` / `MTK_COLLATERAL_FMT_MIN` / `MTK_COLLATERAL_FMT_RATIO` in `.claude/settings.local.json` env | Thresholds for `hooks/collateral-guard.sh`, which flags churn that is not the change you made: whitespace/EOL-only rewrites (a small edit to a CRLF file rewrites the whole file), generated artifacts riding along undeclared (one `npm install` drags a lockfile into a feature commit), asset directories regenerated wholesale, and structured files re-serialized around a small real edit (a big JSON diff whose parsed content barely changed — the class a whitespace comparison cannot see). Wired into `pre-commit-review` step 4.6 and the `spec-drift-detection` collateral axis |
| Write a tracked run receipt | `MTK_RUN_RECEIPT=1` in `.claude/settings.local.json` env | `implement` Phase 7.5 writes `docs/specs/<date>-<slug>.receipt.md` — a **tracked** sibling of the spec holding the run's evidence (baseline vs final figures, gates and their reasons, dispatch path and compensations, ceremony reductions, drift/coverage/collateral verdicts, reviewer lane outcomes). The workflow artifact under `.mtk/` is gitignored, so without this the only tracked residue of a run is `tasks/lessons.md` and the spec. Fields never recorded are written as `not recorded`, never reconstructed |
| Calibrate context-budget nags to your window | `MTK_CONTEXT_WINDOW_TOKENS=200000` in `.claude/settings.local.json` env | Rescales `context-budget.sh`'s file/mod/op nudges from the 1M default to your model's real context window (set 200000 for Haiku 4.5), so the checkpoint/handoff nudges fire at the right point for the model in use. Per-threshold overrides: `MTK_CTX_FILES_WARN` / `MTK_CTX_MODS_WARN` / `MTK_CTX_OPS_WARN`; `MTK_CONTEXT_BUDGET_PCT` sets the read-bytes reset percentage |
